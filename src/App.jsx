...
// ════════════════════════════════════════════════════════════════════════════
function PlayerSearch({ api, onPlayerAdded, toast }) {
  const [query, setQuery] = useState("");
  const [results, setResults] = useState([]);
  const [loading, setLoading] = useState(false);
  const [importing, setImporting] = useState(null); // nhlPlayerId being imported
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    setLoading(true);
    api.get(`/import/search?q=${encodeURIComponent(debouncedQuery.trim())}&sport=NHL`)
      .then(d => {
        if (!cancelled) {
  ...
  // Quick import: create + link + import in one shot
  async function quickImport(nhlPlayer) {
    setImporting(nhlPlayer.id);
    try {
      const d = await api.post("/import/quick", {
        nhlPlayerId: nhlPlayer.id,
        name: nhlPlayer.name,
        team: nhlPlayer.team,
      ...
      if (d.player) {
        toast(`${d.player.name} added with ${d.imported || 0} entries!`, "success");
        onPlayerAdded(d.player);
        setQuery("");
        setResults([]);
        setShowDropdown(false);
      }
      ...
      if (e.message?.includes("already linked")) {
        toast("This player is already in your tracker", "warning");
      } else {
  ...
  return (
    <div ref={wrapperRef} style={{ position: "relative", padding: "8px 12px" }}>
          ...
          {results.map(r => (
            <button
              ...
              style={{
                ...
                width: "100%",
                padding: "10px 12px",
                background: importing === r.id ? "#141926" : "transparent",
            ...
            >
              <div>
                <div style={{ fontSize: 12, color: "#dde1ec", fontWeight: 500 }}>{r.name}</div>
                <div style={{ fontSize: 9, color: "#5a6680", marginTop: 2 }}>
                  {r.team} · {r.position}
                </div>
              </div>
              {importing === r.id ? (
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  <Spinner size={12} />
                  <span style={{ fontSize: 9, color: "#00d4ff" }}>Importing...</span>
                </div>
              ) : (
                <span style={{ fontSize: 8, color: "#3a4258", letterSpacing: 1 }}>+ ADD</span>
              )}
            </button>
          ))}
        </div>
...
function Dashboard({ toast, onLogout }) {
  ...
  // ── NHL Import (legacy — kept for existing linked players) ────────────────
  ...
  // ── Quick add callback (from autocomplete) ────────────────────────────────
  function handlePlayerAdded(newPlayer) {
    setPlayers(prev => [...prev, newPlayer]);
    setSelectedId(newPlayer.id);
    setView("profile");
    // Reload entries for the new player
    setTimeout(() => {
      Promise.all([
        api.get(`/entries?playerId=${newPlayer.id}`),
        api.get(`/stats/${newPlayer.id}`),
      ]).then(([ed, sd]) => {
        setEntries(ed.entries || []);
        setStats(sd.stats);
      }).catch(() => {});
    ...
    <div style={{ fontFamily:"'DM Mono','Courier New',monospace", background:"#07090d", minHeight:"100vh", color:"#dde1ec" }}>
      ...
      <div style={{ display:"flex", minHeight:"100vh" }}>
        ...
        <div style={{ width:220, background:"#0b0e15", borderRight:"1px solid #141926", display:"flex", flexDirection:"column", position:"sticky", top:0, height:"100vh", overflowY:"auto" }}>
          ...
          {/* ── AUTOCOMPLETE SEARCH (replaces old ADD PLAYER button) ── */}
          <PlayerSearch api={api} onPlayerAdded={handlePlayerAdded} toast={toast} />
          ...
          <div style={{ marginTop:"auto", padding:"12px 14px", borderTop:"1px solid #141926", display:"flex", flexDirection:"column", gap:6 }}>
            {player && (
              <>
                ...
                {canImport && player.nhlPlayerId && (
                  <button onClick={()=>setView("import")} className="btn" style={{ width:"100%", background:"#0d1018", color:"#b347ff", padding:"10px 0", fontSize:10, letterSpacing:2, border:"1px solid #b347ff44", borderRadius:4, marginTop:4 }}>
                    ↓ NHL RE-IMPORT
                  </button>
                )}
        ...
        <div style={{ flex:1, overflowY:"auto" }}>
            ...
            <div className="su" style={{ padding:28 }}>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"repeat(5,1fr)", gap:10, marginBottom:20 }}>
                {[
                  ...
                  { label:"WIN RATE (FG)",   val:fmt(localStats.winRateFirst),         color:"#00e676" },
                  { label:"AVG TIME",        val:localStats.avgTime,                   color:"#ffd600" },
                ].map(s => (
                  <div key={s.label} style={{ background:"#0d1018", border:"1px solid #141926", borderRadius:8, padding:16, textAlign:"center" }}>
                    <div style={{ fontSize:7, letterSpacing:2, color:"#3a4258", marginBottom:8 }}>{s.label}</div>
                    <div style={{ fontFamily:s.big?"'Bebas Neue'":"inherit", fontSize:s.big?42:26, color:s.color, lineHeight:1 }}>{s.val}</div>
                  </div>
                ))}
              </div>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14, marginBottom:18 }}>
                ...
                <div style={{ background:"#0d1018", border:"1px solid #141926", borderRadius:8, padding:20 }}>
                  <div style={{ fontSize:9, letterSpacing:2, color:"#3a4258", marginBottom:16 }}>FIRST GOALS — BY SITUATION</div>
                  {SITUATIONS.map(s => {
                    const d = localStats.bySituation?.[s] || { count:0, pctOfFirstGoals:0, pctOfAllGames:0 };
                    return (
                      <div key={s} style={{ marginBottom:14 }}>
                        <div style={{ display:"flex", justifyContent:"space-between", marginBottom:5 }}>
                          <span style={{ fontSize:11, color:SIT_COLOR[s], display:"flex", alignItems:"center", gap:6 }}>
                            <span style={{ width:6, height:6, borderRadius:"50%", background:SIT_COLOR[s], display:"inline-block" }}/>{SIT_LABEL[s]}
                          </span>
          ...
          {/* ── ADD PLAYER (manual — kept as fallback) ── */}
          {view === "add-player" && (
            <div className="su" style={{ padding:28, maxWidth:460 }}>
              <div style={{ fontSize:9, letterSpacing:2, color:"#3a4258", marginBottom:20 }}>NEW PLAYER (MANUAL)</div>
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
          ...
          {/* ── NHL IMPORT (for re-importing existing linked players) ── */}
          {view === "import" && (
            <div className="su" style={{ padding:28, maxWidth:560 }}>
              <div style={{ fontSize:9, letterSpacing:2, color:"#3a4258", marginBottom:6 }}>NHL RE-IMPORT — {player?.name}</div>
              <div style={{ fontSize:10, color:"#5a6680", marginBottom:20 }}>Re-import or update goal data for this player from the NHL API.</div>
          ...
          {!player && view === "profile" && (
            <div style={{ display:"flex", alignItems:"center", justifyContent:"center", height:"100vh", flexDirection:"column", gap:10, color:"#3a4258" }}>
              ...
              <div style={{ fontSize:11 }}>Search for an NHL player in the sidebar to start tracking</div>
            </div>
          )}
        </div>
      </div>
